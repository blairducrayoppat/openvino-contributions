LOG=/c/Users/mrbla/oss/ov_python_build.log
STAT=/c/Users/mrbla/oss/ov_python_build_status.txt
WD=/c/Users/mrbla/AppData/Local/Temp/claude/ovpy-build-watchdog.log
prev_cpu=-1; prev_sz=-1; streak=0; i=0
echo "[$(date '+%H:%M:%S')] ovpy watchdog start" >> "$WD"
while [ $i -lt 60 ]; do
  st=$(tr -d '\r\n' < "$STAT" 2>/dev/null)
  case "$st" in
    *OVPY_BUILD_OK*) echo "[$(date '+%H:%M:%S')] BUILD OK -> exit0" >> "$WD"; exit 0;;
...
echo "[$(date '+%H:%M:%S')] ovpy watchdog CAP -> exit0" >> "$WD"; exit 0
